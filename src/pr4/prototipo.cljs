...
(defn draw-chart [element-id data-array chart-width options]
  #(let [g-charts-instance (new google-charts)
         ...
         data (fn [] (-> g-charts-instance .-api .-visualization
                  (.arrayToDataTable data-array)))
         chart (fn []
                 (when-let [el-width (some-> js/document
                                             (.getElementsByClassName "com-rigsomelight-devcard")
                                             (.item 0)
                                             .-clientWidth
                                             (* chart-width)
                                             (str "px"))]
                   (set! (-> el .-style .-width) el-width))
...
(defonce modality (reagent/atom "Musculação"))
         ...
         [:div
          ...
          [:> material-select
           {:style #js {:paddingLeft "10px"}
            :value @modality
            :onChange #(reset! modality (-> % .-target .-value))}
           [:> material-menu-item {:value "CrossFit"} "CrossFit"]
           [:> material-menu-item {:value "Musculação"} "Musculação"]
           [:> material-menu-item {:value "Ciclismo"} "Ciclismo"]
           [:> material-menu-item {:value "Corrida"} "Corrida"]
           [:> material-menu-item {:value "Natação"} "Natação"]]
          ]
         [:div [:div#chart-div
                {:style #js {:paddingTop "20px" :height 200}}]]
...
(defcard-rg Antigo-input-de-sessão
  (fn [devcard-data _]
    (let [{:keys [hidden?]} @devcard-data]
      (swap! devcards-hidden conj hidden?)
      [:<>
      ...
      [:<>
       ...
       [:div.card-container
        ...
        [:div.component-container
         ...
         [:h3
          {:style #js {:textAlign "center"}}
          "Microcíclo " (let [[session-sel-start session-sel-end] @(re-frame/subscribe [::app-state/session-selection])
                          ...
                          (str (when micro-sel-start (inc micro-sel-start)) #_"-" #_micro-sel-end))
          " "]
...
;;   #(into [ext-store/firebase-el {:user "tcho"])
;;   {}
;;   {:frame false
;;    :heading false})
